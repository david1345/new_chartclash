        ...
        for (const key of groupKeys) {
            ...
            try {
                ...
                if (isCrypto) {
                    try {
                        ...
                        if (cryptoData.Response === 'Success' && cryptoData.Data?.Data?.length > 0) {
                            ...
                            // 1분 베팅의 경우 마감 시각 그대로 검색, 그 외에는 1분 전 캔들의 종가를 참고
                ...
                if (openPrice === null || closePrice === null) {
                    ...
                    if (closePrice === null) {
                        closePrice = await fetchClosePrice(group.symbol, group.tf, group.openTime, group.closeTime, avgEntry);
                ...
                for (const pred of group.preds) {
                    ...
                    for (let attempt = 0; attempt < 3; attempt++) {
                        const { data, error } = await supabase.rpc('resolve_prediction_advanced', {
                            p_id: pred.id,
                            p_close_price: closePrice, // Second param in SQL
                            p_open_price: openPrice    // Third param in SQL
                        });
...
async function fetchClosePrice(symbol: string, timeframe: string, openTime: number, closeTime: number, referencePrice: number): Promise<number> {
    ...
    if (cryptoPairs.includes(cleanSymbol) || cleanSymbol.endsWith('USDT')) {
        ...
        // --- PRIORITY 1: CryptoCompare (Most Reliable for Serverless) ---
        try {
            const ts = Math.floor(closeTime / 1000); // Use Close Time
        ...
        // --- PRIORITY 3: Legacy Exchanges (Exchange Fallback) ---
        const exchanges = [
            { name: 'Bybit', url: `https://api.bybit.com/v5/market/kline?category=spot&symbol=${cleanSymbol}&interval=1&start=${openTime}&limit=1`, parser: (d: any) => d?.result?.list?.[0]?.[4] },
            { name: 'MEXC', url: `https://api.mexc.com/api/v3/klines?symbol=${cleanSymbol}&interval=1m&startTime=${openTime}&limit=1`, parser: (d: any) => d?.[0]?.[4] }
        ];
        ...
        for (const ex of exchanges) {
            try {
                ...
                console.warn(`${getTimestamp()} ${ex.name} failed:`, e);
            }
        }
        ...
        // 시드가 openTime에만 고정되어 시작/마감가가 같아지는 현상 방지 (closeTime 사용)
