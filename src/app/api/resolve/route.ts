...
// Helper to get user identifiers
...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const userIds = predictions ? [...new Set(predictions.map(p => p.user_id))] : [];
        ...
        for (const pred of predictions) {
            ...
            if (isReady) {
                const username = usernameMap[pred.user_id] || 'Unknown';
                console.log(`${getTimestamp()} [👤 ${username}] [Resolution] Pred ${pred.id} (${pred.timeframe}): CloseTime ${new Date(closeTime).toISOString()} is READY.`);
        ...
        for (const key of groupKeys) {
            ...
            try {
                ...
                if (openPrice === null || closePrice === null) {
                    ...
                    if (openPrice === null) {
                        openPrice = await fetchClosePrice(group.symbol, group.tf, group.openTime, group.openTime, avgEntry);
                        console.log(`${getTimestamp()} [Resolution] Fallback openPrice result: ${openPrice}`);
                    }

                    if (closePrice === null) {
                        // Use closeTime for close price fetching
                        closePrice = await fetchClosePrice(group.symbol, group.tf, group.closeTime, group.closeTime, avgEntry);
            ...
            if (openPrice !== null && closePrice !== null) {
                for (const pred of group.preds) {
                    ...
                    if (success) {
                        ...
                        const username = usernameMap[pred.user_id] || 'Unknown';
                        console.log(`${getTimestamp()} [👤 ${username}] Successfully resolved prediction ${pred.id} (Fairness Model Applied)`);
                    } else {
                        ...
                        if (lastError !== 'Already resolved') {
                            ...
                            // Still count it as processed detail but don't log as error
...
// Helper: Fetch Price (Multi-Provider with Failover)
async function fetchClosePrice(symbol: string, timeframe: string, targetTime: number, closeTime: number, referencePrice: number): Promise<number> {
    const cleanSymbol = symbol.replace('/', '').toUpperCase();

    // 0. Helper for Timeout
    const fetchWithTimeout = async (url: string, timeout = 5000) => { // 5s timeout
    ...
    if (cryptoPairs.includes(cleanSymbol) || cleanSymbol.endsWith('USDT')) {
        ...
        const exchangesList = [
            { name: 'Binance', url: `https://api.binance.com/api/v3/klines?symbol=${cleanSymbol}&interval=1m&startTime=${targetTime}&limit=1`, parser: (d: any) => d?.[0]?.[4] },
            { name: 'Bybit', url: `https://api.bybit.com/v5/market/kline?category=spot&symbol=${cleanSymbol}&interval=1&start=${targetTime}&limit=1`, parser: (d: any) => d?.result?.list?.[0]?.[4] },
            { name: 'MEXC', url: `https://api.mexc.com/api/v3/klines?symbol=${cleanSymbol}&interval=1m&startTime=${targetTime}&limit=1`, parser: (d: any) => d?.[0]?.[4] }
        ];
        ...
        for (const ex of exchangesList) {
            try {
                ...
                if (res.ok) {
                    ...
                    if (price) {
                        console.log(`${getTimestamp()} [Price] Fetched from ${ex.name}: ${price}`);
        ...
        // Prevent identical prices by using closeTime in seed
        ...
        try {
            ...
            if (data) {
                ...
                if (result) {
                    ...
                    // For range=7d, we check the latest price if it's "close enough" (within 5 mins)
                    if (bestIdx !== -1 && minDiff < 300 && closes[bestIdx] != null) {
