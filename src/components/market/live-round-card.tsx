...
import { Flame, Trophy, Users, TrendingUp, TrendingDown, Sparkles } from "lucide-react";
...
export function LiveRoundCard({
    ...
    return (
        ...
        >
            <Card
                onClick={() => router.push(`/play/${assetSymbol}/${timeframe}`)}
                className="group relative overflow-hidden transition-all duration-500 h-full flex flex-col justify-between p-4 border-white/5 bg-white/5 hover:bg-white/[0.08] hover:border-white/20 cursor-pointer"
            >
                {/* Header */}
                <div className="flex justify-between items-start mb-3">
                    ...
                    {aiDirection && (
                        <div
                            onClick={(e) => {
                                e.stopPropagation();
                                router.push(`/community?tab=analyst-hub&asset=${assetSymbol}&timeframe=${timeframe}`);
                            }}
                            className="flex items-center gap-1 px-2 py-1 rounded-full bg-purple-500/10 border border-purple-500/20 hover:bg-purple-500/20 hover:border-purple-500/40 transition-all group/ai z-10"
                        >
