...
export function TrendingMarketSection({ searchQuery = "", selectedCategory = "CRYPTO" }: TrendingMarketSectionProps) {
    ...
    const fetchData = async () => {
        try {
            ...
            if (data.success && data.data && data.data.length > 0) {
                ...
                // Fallback: Fetch latest AI Analyst Hub data if no live rounds
            ...
            // Try AI Analyst fallback on error
            await fetchAIAnalystFallback();
        } finally {
            setLoading(false);
        }
    ...
    const fetchAIAnalystFallback = async () => {
        try {
            ...
            // Get top 3 recent rounds from AI Analyst Hub
            const recentRounds = rounds.slice(0, 3);
            const fallbackData = recentRounds.map((round: any) => ({
                asset_symbol: round.asset_symbol || 'BTCUSDT',
                timeframe: round.timeframe || '15m',
                asset_name: 'Bitcoin', // Default, should be mapped properly
                asset_type: 'CRYPTO',
            ...
            setTrendingAssets(fallbackData);
        } catch (error) {
