...
export function StatsPanel({ assetSymbol, timeframe }: StatsPanelProps) {
    ...
    useEffect(() => {
        ...
        const fetchStats = async (isInitial = false) => {
            // Quiet update on real-time to avoid flickering
        ...
        const channel = supabase
            ...
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'predictions' }, () => {
                // Debounce slightly to avoid rapid chart re-renders
                const timer = setTimeout(() => fetchStats(false), 2000);
                return () => clearTimeout(timer);
            })
            .subscribe();

        // Refresh when round changes (Auto-reset)
        const interval = setInterval(() => fetchStats(false), 15000); // Check every 15s
