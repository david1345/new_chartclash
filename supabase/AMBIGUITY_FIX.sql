-- [RPC AMBIGUITY FIX]
-- This script removes duplicate versions of the resolution function that are causing "Could not choose the best candidate" errors.

-- 1. Remove ALL existing versions to clear the path
...
-- 2. Restore the definitive version (0.5% Fee logic)
...
begin
    ...
    -- 2. Fetch Configs from system_settings (0.5% Fee must be set there)
    SELECT (value->>0)::numeric INTO v_house_edge FROM system_settings WHERE key = 'house_edge'; -- 0.995
    SELECT (value->>0)::numeric INTO v_base_ratio FROM system_settings WHERE key = 'base_profit_ratio'; -- 0.8
    ...
    -- Default to 0.995 if not set
    v_house_edge := COALESCE(v_house_edge, 0.995);
    v_base_ratio := COALESCE(v_base_ratio, 0.8);
