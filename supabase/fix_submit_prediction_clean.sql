-- Clean up duplicate submit_prediction functions and create the correct one

-- 1. Drop all existing versions (both parameter orders)
DROP FUNCTION IF EXISTS public.submit_prediction(uuid, text, text, text, integer, numeric, numeric);
DROP FUNCTION IF EXISTS public.submit_prediction(uuid, text, text, text, numeric, numeric, integer);
DROP FUNCTION IF EXISTS public.submit_prediction(p_user_id uuid, p_asset_symbol text, p_timeframe text, p_direction text, p_bet_amount integer, p_target_percent numeric, p_entry_price numeric);
DROP FUNCTION IF EXISTS public.submit_prediction(p_user_id uuid, p_asset_symbol text, p_timeframe text, p_direction text, p_target_percent numeric, p_entry_price numeric, p_bet_amount integer);

-- 2. Create the correct version with proper candle_close_at calculation
