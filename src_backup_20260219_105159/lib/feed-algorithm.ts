...
export interface AuthorStats {
    tier: Tier
    assetAccuracy: number // 0 ~ 1 (예: 0.63 = 63%)
}
...
export interface PostEngagement {
    ...
    comments: number
    shares: number
    likedByHighTier: number // 상위 티어 유저 반응 수
}

export interface PostMeta {
    createdAt: Date
    volatilityTarget: number // 예: 0.5, 1, 2 (%)
}
...
// 🧠 점수 계산 메인 함수 (Hybrid Architecture)

/**
 * 1. Base Score (Server/DB Side)
 * - DB에 저장될 점수입니다. 개인화 요소(Follow)를 제외한 객관적 지표만 포함합니다.
 * - 좋아요/댓글 등의 변화가 있을 때 재계산(Recompute) 되어 DB에 갱신됩니다.
 */
