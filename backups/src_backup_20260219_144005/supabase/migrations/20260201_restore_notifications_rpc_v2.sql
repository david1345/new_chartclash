-- RESTORE RPC with Title Fix
-- Error was: null value in column "title" violates not-null constraint
-- Fix: Insert 'title' field
...
BEGIN
    ...
    ELSE
        v_title := 'Prediction Ended (No Decision)';
    END IF;
    
    -- 6. Create Notification (FIXED: Added title)
    INSERT INTO notifications (user_id, type, title, message, prediction_id)
    VALUES (
        v_prediction.user_id,
        CASE WHEN v_status = 'WIN' THEN 'win' WHEN v_status = 'LOSS' THEN 'loss' ELSE 'info' END,
        v_title,
