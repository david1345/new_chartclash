-- 🔔 Notification Messaging Overhaul
-- Migration: 20260210184000_overhaul_notification_logic.sql
...
BEGIN
    ...
    IF v_is_dir_correct THEN
        ...
        IF v_is_target_hit AND v_prediction.target_percent > 0 THEN
            v_notif_message := '완벽합니다! 방향과 타겟 모두 적중! 🎯';
            ...
            END IF;
        ELSE
            v_notif_message := '방향은 맞았어요! 타겟까지 맞추면 더 큰 보너스가 있습니다! 💪';
        END IF;

        -- Final Calculation (Simplified for this version)
    ...
    ELSIF v_price_change = 0 THEN
        v_status := 'ND';
        v_notif_type := 'info';
        v_notif_message := '휴, 다행입니다! 가격 변화가 없어 보증금이 반환되었습니다. 🛡️';
    ...
    ELSE
        v_status := 'LOSS';
        v_notif_type := 'loss';
        v_notif_message := '예측이 빗나갔습니다. 다음 기회에 꼭 승리하세요! 화이팅! 🍀';
        v_payout := 0;
    ...
    -- Notification Title with Timeframe
    v_notif_title := format('%s (%s) 결과', v_prediction.asset_symbol, v_prediction.timeframe);
    ...
    VALUES (
        ...
        v_notif_type, 
        v_notif_title, 
        format('%s (+%s pts)', v_notif_message, ROUND(v_final_profit_pts)),
