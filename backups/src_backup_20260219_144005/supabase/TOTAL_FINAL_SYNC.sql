-- ==============================================================================
-- 🚀 CHARTCLASH - UNIFIED SYSTEM SYNC (2026-02-16)
-- Run this in Supabase SQL Editor to enable AI Analysts & System Settings
-- ==============================================================================

-- 1. [PROFILES] AI Identity
...
-- 2. [PREDICTIONS] AI Channel Support
...
-- 3. [SYSTEM SETTINGS] Management Table
...
VALUES 
    ('house_edge', '0.9', 'Platform payout multiplier (0.9 = 10% fee)'),
    ('base_profit_ratio', '2.0', 'Base profit multiplier'),
    ('tf_multipliers', '{"1m": 1.0, "5m": 1.2, "15m": 1.5, "1h": 2.0, "4h": 3.0, "1d": 5.0}', 'Timeframe-based multipliers'),
    ('target_bonuses', '[{"max": 0.5, "bonus": 50}, {"max": 1.0, "bonus": 120}, {"max": 1.5, "bonus": 200}, {"max": 2.0, "bonus": 300}]', 'Target percentage bonuses'),
    ('streak_multipliers', '{"1": 1.0, "2": 1.05, "3": 1.1, "4": 1.15, "5": 1.25}', 'Streak-based bonus multipliers')
ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value;

-- 4. [RPC] Updated Reward Engine (Connected to system_settings)
...
begin
    ...
    INSERT INTO public.notifications (user_id, type, message, points_change, is_read) VALUES (v_prediction.user_id, 'prediction_resolved', format('%s prediction: %s (%s pts)', v_prediction.asset_symbol, v_status, v_profit_change), v_profit_change, FALSE);
