...
export default function MatchHistoryPage() {
    ...
    const fetchHistory = async () => {
        ...
        if (isImpersonating) {
            console.log("👻 HISTORY: GHOST MODE - Viewing history for", targetId);
        }
        ...
        if (data) {
            ...
            const historyWithStreaks = sorted.map(pred => {
                ...
                const close = pred.actual_price || 0;
                const changePct = entry > 0 && close > 0 ? ((close - entry) / entry) * 100 : 0;
                const isTargetHit = Math.abs(changePct) >= pred.target_percent;
                ...
                if (pred.status === 'WIN') {
                    if (isGreenZone && isTargetHit) {
                        ...
                        // WIN but not "Perfect": Reset streak
                        currentStreak = 0;
            ...
            setPredictions(historyWithStreaks.reverse());
        }
    };
...
function HistoryRow({ pred }: { pred: any }) {
    ...
    // Calculate actual move % for display
    const entry = pred.entry_price || 0;
    // New logic uses 'close_price'. Fallback to 'actual_price' for old records.
    const close = pred.close_price || pred.actual_price || 0;

    // New logic uses 'actual_change_percent'. Fallback to local calc.
    const changePct = pred.actual_change_percent ?? (entry > 0 && close > 0 ? ((close - entry) / entry) * 100 : 0);

    // Logic for Target Hit: If actual_change was recorded, assume it's accurate. If not, calc locally.
    ...
    // Profit Display
    const profitValue = pred.profit_loss ?? pred.profit ?? 0;
