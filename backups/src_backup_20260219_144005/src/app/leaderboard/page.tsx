...
export default function LeaderboardPage() {
        ...
        const channel = supabase
            ...
            .on(
                ...
                (payload) => {
                    ...
                    // Refresh the entire leaderboard to ensure correct sorting
                    fetchLeaderboard();
                }
            )
            .subscribe();
    ...
    const fetchLeaderboard = async () => {
        ...
        console.log("[Leaderboard] Profiles received:", profiles?.length);
        ...
        if (profiles) {
            const enriched = profiles.map((p, i) => ({
                ...
                // Use the most up-to-date field or best ever
                streak: Math.max(p.streak || 0, p.streak_count || 0)
            }));
            setLeaders(enriched);
            console.log("[Leaderboard] Enriched leaders set.");

            // Identify current user
            const { data: { user } } = await supabase.auth.getUser();
            console.log("[Leaderboard] Current auth user:", user?.id);

            if (user) {
                // Find user in the top 100
                const myProfile = enriched.find(p => p.id === user.id);
                if (myProfile) {
                    console.log("[Leaderboard] User found in top 100.");
                    setUserRank(myProfile);
                } else {
                    console.log("[Leaderboard] User not in top 100, fetching rank via RPC...");
                    // If not in top 100, fetch user's specific rank via RPC
                    const { data: rank, error: rankErr } = await supabase.rpc('get_user_rank', { p_user_id: user.id });
                    const { data: myData, error: dataErr } = await supabase
                        .from('profiles')
                        .select('id, username, points, total_games, total_wins, streak_count, streak, tier')
                        .eq('id', user.id)
                        .single();

                    if (rankErr) console.error("[Leaderboard] RPC Error:", rankErr);
                    if (dataErr) console.error("[Leaderboard] Profile Error:", dataErr);
                    ...
                    if (myData) {
                        ...
                        console.log("[Leaderboard] User rank and data set.");
                    }
                }
            }
        }
